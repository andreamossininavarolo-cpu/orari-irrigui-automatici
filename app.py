...
# --- Configurazione Pagina e Stile per Compattezza ---
st.set_page_config(page_title="Turni Navarolo", layout="centered")

st.markdown("""
<style>
    /* Riduce la dimensione del titolo principale */
    h1 {
        font-size: 1.8em !important;
        margin-bottom: 0px !important;
        padding-bottom: 0px !important;
    }
    /* Forza le 3 colonne dei pulsanti a stare sempre sulla stessa riga anche su mobile */
    [data-testid="column"] {
        width: calc(33.3333% - 8px) !important;
        flex: 1 1 calc(33.3333% - 8px) !important;
        min-width: calc(33% - 8px) !important;
    }
    /* Riduce lo spazio tra gli elementi */
...
if 'data_selezionata' not in st.session_state:
    # Usiamo il 15/04/2026 come giorno di partenza predefinito per i test della stagione 2026
    st.session_state.data_selezionata = datetime(2026, 4, 15).date()
...
if not df_risultato.empty:
    ...
    # --- PULSANTI DI NAVIGAZIONE RAPIDA (FORZATI IN ORIZZONTALE) ---
    ...
    # Calendario di controllo (nascosto il titolo per risparmiare spazio)
    ...
    if turni_del_giorno.empty:
        ...
        for _, turno in turni_del_giorno.iterrows():
            ...
            # Pallini fluorescenti ad alto impatto visivo
    ...
    with st.expander("📥 Scarica Tabellone Stagionale (CSV/Excel)"):
